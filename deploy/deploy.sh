...
for digest in "$generator_digest" "$router_digest" "$code_server_digest" "$code_server_vnc_digest"; do
...
python - "$namespace" "$generator_digest" "$router_digest" "$render_dir" <<'PY'
import sys
from pathlib import Path

namespace, generator_digest, router_digest, output = sys.argv[1:]
sources = (
...
printf '{"data":{"CODE_SERVER_IMAGE":"harbor.jbnu.ac.kr/jdevops/code-server@%s","CODE_SERVER_VNC_IMAGE":"harbor.jbnu.ac.kr/jdevops/code-server-vnc@%s"}}\n' \
  "$code_server_digest" "$code_server_vnc_digest" > "$config_patch"
